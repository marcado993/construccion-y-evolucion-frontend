...
export default function BrailleToText() {
  ...
  const textareaRef = useState<{ current: HTMLTextAreaElement | null }>({ current: null })[0]; // Quick hack for ref if useRef not imported, wait... I should import useRef.
  ...
  const insertAtCursor = (text: string) => {
    ...
    if (!textarea) {
      setBrailleInput(prev => prev + text);
      return;
    }
